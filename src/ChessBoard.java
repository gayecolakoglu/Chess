...
//then it puts the initial pieces to their locations
    public ChessBoard() {
        for(int i=0; i<8; i++) {
            char letter = 'A';
            for(int j=0; j<8; j++) {
                Square square = new Square("" + letter,"" + (8-i), this);
                board[i][j] = square;
                letter++;
            }
        }
        ...
        for(int i : array) {
            for(int j : array) {
                board[i][j].piece = new Rook((i == 7) ? 0 : 1, board[i][j]);
            }
        }
        ...
        for(int i : array) {
            for(int j : array2) {
                board[i][j].piece = new Knight((i == 7) ? 0 : 1, board[i][j]);
            }
        }
        ...
        for(int i : array) {
            for(int j : array2) {
                board[i][j].piece = new Bishop((i == 7) ? 0 : 1, board[i][j]);
            }
        }

        for(int i=0; i<2; i++) {
            board[array[i]][3].piece = new Queen((i == 0) ? 0 : 1, board[array[i]][3]);
            board[array[i]][4].piece = new King((i == 0) ? 0 : 1, board[array[i]][4]);
        }
